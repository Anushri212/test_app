...
def page_build_and_train():
    ...
    model_checkpoint_path = 'final_epoch_model.h5'

    # Check if model checkpoint file exists
    if os.path.exists(model_checkpoint_path):
        st.session_state['model'] = load_model(model_checkpoint_path)
        st.write("Model loaded from checkpoint.")
        st.write(f"Model checkpoint path: {model_checkpoint_path}")
    else:
        if st.button("Train Model"):
            st.write("Starting model training...")  # Log message to indicate training has started
            try:
                ...
                model.add(Dense(st.session_state['output_sequence_length'], activation='linear'))
                model.compile(optimizer=optimizer, loss=loss_function)
                early_stopping = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
                epoch_time_callback = EpochTimeCallback()
                model_checkpoint = ModelCheckpoint(model_checkpoint_path, save_best_only=True, monitor='val_loss', mode='min')

                history = model.fit(st.session_state['trainX'], st.session_state['trainY'], epochs=epochs, batch_size=batch_size, validation_split=0.2, callbacks=[early_stopping, epoch_time_callback, model_checkpoint])
                st.session_state['model'] = model

                st.write("Model training completed")
                st.write("Model saved at", model_checkpoint_path)
                st.write(f"Training loss: {history.history['loss'][-1]:.4f}")
                st.write(f"Validation loss: {history.history['val_loss'][-1]:.4f}")
                st.write(f"Best model saved at epoch: {early_stopping.best_epoch}")
        ...
        for msisdn in random_msisdns:
            ...
            if len(lstm_data) >= st.session_state['sequence_length'] + st.session_state['output_sequence_length']:
                ...
                if rolling_prediction:
                    ...
                    st.pyplot(plt)
                    # Calculate RMSE
                    rmse = np.sqrt(mean_squared_error(actuals_msisdn_inverse, predictions_msisdn_inverse))
                    st.write(f"RMSE for MSISDN {msisdn}: {rmse:.2f}")
...
def main():
    st.sidebar.title("Navigation")
    page = st.sidebar.selectbox("Select a page", ["Load and Preprocess Data", "Build and Train Model", "Generate Predictions"])
